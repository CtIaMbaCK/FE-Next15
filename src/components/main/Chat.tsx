  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/chat`, {
        ...
        body: JSON.stringify({ query: trimmed }),// tuy chinh theo cai ben api
          ...
          <AnimatePresence initial={false}>
            {isVisible ? (
              <motion.div
                ...
                key="box"
              >
                <Box className="bg-white lg:w-[98%] lg:h-[96%] w-[96%] h-[98%] rounded-2xl flex flex-col shadow-xl/30">
                  {/* head */}
                      ...
                      {chatMessages.map((chat, index) => (
                        chat.sender === "user" ? (
                          <Box key={index} className="flex w-full justify-end mb-2">
                            <Box className="flex items-start gap-2 p-4 max-w-[60%]">
                          ...
                          <Box key={index} className="flex w-full justify-start mb-2">
                            <Box className="flex items-start gap-2 p-4 max-w-[80%]">
                              <Box className="bg-[#1565c0] rounded-full w-8 h-8 flex items-center justify-center cursor-pointer">
                                <PersonIcon className="text-white" />
                              </Box>
                              <Box className="bg-[#1565c0] rounded-lg p-2">
                  ...
                  <Box className="flex items-center p-4 shadow-xl justify-center w-full relative gap-3">
                    ...
                    />
                    <Box className="bg-[#B02E35] hover:scale-110  w-8 h-8 rounded-full flex items-center justify-center right-6 bottom-6 cursor-pointer">
                      <SendIcon
